...
* **Plantillas de Correo y Salesforce:** Sección **Plantillas** alimentada por la hoja `Plantillas` (columnas: `Titulo | Tipo | Asunto | Cuerpo | Consideraciones`). El portal interpreta cada fila según su **Tipo**:
  ...
  * **Sales Force** — muestra solo *Cuerpo* + *Consideraciones* (el asunto se ignora).
  ...
  Las plantillas se crean y editan desde la hoja con el menú **✉️ Plantillas → Abrir constructor…**, que abre un *sidebar* (`ConstructorPlantillas.html`) con vista previa en vivo, botón para insertar campos `[…]`, y lista de plantillas existentes (editar / duplicar / borrar). El constructor administra una columna `ID` para identificar cada fila; el portal la ignora.
* **Mis Colecciones:** Cada herramienta, plantilla, formato, presentación o pago tiene una **estrella ⭐** para guardarla en colecciones propias (p. ej. *“Mi turno”*, *“Devoluciones”*). Las colecciones aparecen en el dashboard y agrupan elementos de **distintas secciones** en un solo bloque; se guardan en `localStorage` (`ventel-collections`), sin tocar la hoja.
* **Copiados Recientes:** Un botón flotante abre un cajón con el **historial de lo último que copiaste** (claves, CLABEs, asuntos, cuerpos, correos…) para reutilizarlo de un clic. Se guarda local (`ventel-clips`, máx. 12).
* **Abrir en Gmail (plantillas de correo):** Las plantillas de tipo *Correo* tienen un botón **“Abrir en Gmail”** que abre la ventana de redacción con el **asunto y el cuerpo ya rellenados**. Además, los campos entre corchetes `[ así ]` ahora también funcionan en el **Asunto**, no solo en el cuerpo.
* **Buscar = filtrar en pantalla:** Al elegir un resultado del buscador global, la sección destino se **filtra a las coincidencias** y deja **primero las que coinciden por nombre**, además de resaltar la elegida (antes solo “saltaba” a un elemento).
* **Ajustes de Vista:** Desde la barra lateral (*Ajustes de vista*) el asesor elige **densidad** (cómodo/compacto), **tamaño de texto** (4 niveles) y **alto contraste**. Todo se recuerda localmente (`ventel-density`, `ventel-textscale`, `ventel-contrast`).
* **Estado de Conexión (offline):** Si se pierde internet, el portal sigue mostrando los **últimos datos guardados** y avisa con un indicador *“Sin conexión”*; al reconectar, revalida solo.
* **Programación Visible de Anuncios:** El constructor de anuncios muestra en lenguaje claro **qué pasará con la publicación** (*“Se publica ya y no caduca”*, *“Programado para el 25 jun”*, *“Activo · expira en 2 días”*, *“⚠️ Expira HOY”*) y marca en la lista las publicaciones **por expirar**.
